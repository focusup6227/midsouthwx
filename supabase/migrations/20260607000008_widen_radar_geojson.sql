-- Radar map: drop the Mid-South bounding-box filter so the radar shows all
-- active CONUS alerts (matches what reference weather apps display). Bump
-- limit 150 → 500 so nationwide outbreaks don't get clipped. The dispatcher
-- (which actually sends Telegram alerts to subscribers) is independent of
-- this RPC, so widening here doesn't broaden delivery.
