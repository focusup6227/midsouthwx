...
export default defineJob('couplet-poll', async (req) => {
  ...
  async function worker(rBase: string, rToken: string): Promise<void> {
    while (queue.length > 0) {
      const site = queue.shift();
      if (!site) return;
      results.push(await scanOneSite(site, rBase, rToken, supa));
    }
  }
  await Promise.all(
    Array.from({ length: Math.min(CONCURRENCY, MIDSOUTH_SITES.length) }, () => worker(base, token)),
  );
