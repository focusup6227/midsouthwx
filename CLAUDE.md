...
- **Edge Functions** (`supabase/functions/`, Deno, excluded from tsconfig):
  ...
  - `telegram-webhook` — verifies `X-Telegram-Bot-Api-Secret-Token` header, routes `/start <token>`, location updates, replies, callback queries, `/test` (sample alert + notification-setup guide), `/share` (forwardable invite); writes `replies` + `check_in_responses`; self-notifies operator on distress keywords.
...
- The Supabase config (`supabase/config.toml`) sets `verify_jwt = false` on webhook, signup, send-worker, scheduled-dispatcher, **nws-poll**, **nws-dispatcher**, and the polling workers (lsr/spc/afd/librewxr/couplet-poll, cap/event-recap dispatchers, health-monitor, daily-forecast); each enforces its own auth (HMAC for the webhook, optional cron JWT for workers/dispatchers/poll, rate-limit + Turnstile for signup).
...
- Telegram has no read receipts for bots — "delivered" = 200 from `sendMessage`. Don't promise "read" in UI copy. Bots also **cannot override silent/DND** — `/test` educates subscribers on Android channel overrides / iOS Focus allow-listing, with the WEA + weather-radio disclaimer.
- **Subscriber follow-ups** (migration `20260622000001`): `dispatch_all_clears()` (5-min SQL cron) sends a one-time "all clear" to recipients of an ended warning whose location has no other live warning (source `scheduled` → silent in quiet hours, never outbreak-aggregated); `dispatch_severe_day_notices()` (hourly 12:40–23:40 UTC) sends a once-per-CT-day SPC Day-1 heads-up — ENH+ to all active subscribers in the polygon, SLGT only to watches opt-ins; dedup in `severe_day_notices`.
- Broadcast suite: `/broadcast` is a self-contained YouTube/OBS toolkit (no YouTube API). The console (`app/broadcast/BroadcastConsole.tsx`) drafts a spoken script + YouTube title/description/tags via `lib/ai/broadcast-script.ts` (DeepSeek, `BROADCAST_SCRIPT_SYSTEM`) over `gatherBroadcastContext()` (SPC/AFD/alerts/LSRs), builds OBS browser-source URLs, and previews a 1280×720 thumbnail. **OBS browser sources are cookieless**, so `app/broadcast/overlay/*` (lower-third, ticker, bug — transparent bg, polling `/api/broadcast/state`) and `/api/broadcast/state` + `/cards/broadcast-thumb` are on the **`middleware.ts` public allowlist** and serve **public NWS data only** (the same surface as `/cards/*`). The generated script is shared with the teleprompter (`/broadcast/teleprompter`) via `localStorage` (`lib/broadcast/script-store.ts`), no DB table. Nothing auto-publishes — drafts only.
- Radar: `/radar` route provides live multi-product NEXRAD with Mapbox dark OSM Streets style (free token via NEXT_PUBLIC_MAPBOX_TOKEN; native state/county/city labels + boundaries). Products: NCEP GeoServer / LibreWxR tiles by default (LibreWxR data CC-BY-4.0, attributed via Mapbox attribution control); **Hi-Res (Level II)** uses on-demand Py-ART renders from the sibling `midsouthwx-radar-renderer` service on Fly.io (`RENDERER_BASE_URL` + `RENDERER_TOKEN` in `.env.local`, proxied via `app/api/radar/level2/[site]/route.ts` → PNGs in Supabase `radar-tiles`). Includes Mid-South radar site chooser (fly-to), circle/polygon draw for area alerts via extended `resolve_audience` (geometry), subscriber pins from server GeoJSON. Dashboard links to it. **Traffic cams**: TDOT SmartWay cameras as an opt-in layer (`/api/radar/traffic-cams` proxies TDOT's open-data API with a 10-min cache; click a dot → auto-refreshing snapshot card). Arkansas cams are deliberately excluded — iDriveArkansas ToS prohibits third-party embedding.
