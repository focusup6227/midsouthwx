...
npm run fn:deploy <name>   # deploy a single Edge Function (signup | telegram-webhook | telegram-send-worker | scheduled-dispatcher | nws-poll | nws-dispatcher | lsr-poll | spc-poll | afd-poll | librewxr-poll | cap-dispatcher | health-monitor | event-recap | couplet-poll)
...
- The Supabase config (`supabase/config.toml`) sets `verify_jwt = false` on webhook, signup, send-worker, scheduled-dispatcher, **nws-poll**, **nws-dispatcher**, and the polling workers (lsr/spc/afd/librewxr/couplet-poll, cap/event-recap dispatchers, health-monitor); each enforces its own auth (HMAC for the webhook, optional cron JWT for workers/dispatchers/poll, rate-limit + Turnstile for signup).
...
- Broadcast suite: `/broadcast` is a self-contained YouTube/OBS toolkit (no YouTube API). The console (`app/broadcast/BroadcastConsole.tsx`) drafts a spoken script + YouTube title/description/tags via `lib/ai/broadcast-script.ts` (DeepSeek, `BROADCAST_SCRIPT_SYSTEM`) over `gatherBroadcastContext()` (SPC/AFD/alerts/LSRs), builds OBS browser-source URLs, and previews a 1280×720 thumbnail. **OBS browser sources are cookieless**, so the transparent overlays `app/broadcast/overlay/*` (lower-third, ticker, bug, **breaking** takeover — polling `/api/broadcast/state`), the opaque full-frame scenes `app/broadcast/scene/*` (standby/brb/outro/**headlines** board), `/api/broadcast/state`, and `/cards/broadcast-thumb` are on the **`middleware.ts` public allowlist** and serve **public NWS data only** (the same surface as `/cards/*`). **Live control**: overlays opened with `?live=1` follow the `public.broadcast_control` singleton (operator-edited in the console via `updateControlAction`, read into state by `gatherBroadcastState`) for lower-third text + per-overlay visibility, ~5 s latency. Data/types live in `lib/broadcast/data.ts`. The generated script is shared with the teleprompter (`/broadcast/teleprompter`) via `localStorage` (`lib/broadcast/script-store.ts`). The **Radar capture** preset just deep-links `/radar?hide=1&sub=0&…` (clean-UI mode, subscriber pins forced off) — that radar page stays auth-gated, so it's a screen-capture target, not an OBS browser source. Nothing auto-publishes — drafts only.
